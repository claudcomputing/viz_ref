---
title: 'Graphics in R Session 1: Exploring Data Visually - Key'
...
Activity - layers included in graphic:

```{r activity1a}
knitr::include_graphics(file.path(workDir,"img/session1/graphics-componentsv2-700pxwide.png"))
```
...
> Certain  analysis require certain distributions
> Need best fitting distribution to calculate percentiles, probabilities and make inferences about population
> What proportion of values are associated with important regions
...
```{r activity3a}
fig_bp_size <- ggplot(prof, aes(x = "", y = avg_cls_students)) + 
...
**Plot 1: Overlay scatter plot using `geom_point()` layer**
...
**Plot 2:  Overlay scatter plot using `geom_jitter()` layer**
...
```{r activity4b}
ggplot(prof, aes(x = '', y = avg_cls_students)) +
...
**Plot 3: Create a density plot and compare to boxplot**
...
ggplot(prof, aes( x = avg_cls_students)) + 
...
* **To get started:**
  ...
  3. Add the parameters `title = "my title"`, `ylab = "my y axis label"` to the `lab()` layer
...
* **Tips to get started:**
  ...
  5. Change the plot title and axes labels by adding a `lab()` layer
